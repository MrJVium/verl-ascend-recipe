...
MODEL_PATH="${MODEL_PATH:-/home/model/qwen3_30b_distill_v4_sandboxv3_8k_step552_bf16}"

export KERNELGYM_SERVER_URL="${KERNELGYM_SERVER_URL:-http://127.0.0.1:8002}"
export KERNELGYM_FEEDBACK_MODE=coach
...
# ---------- Resource pools ----------
NNODES_TRAIN="${NNODES:-1}"
NNODES_ROLLOUT="${NNODES:-1}"
N_GPUS_ROLLOUT="${N_GPUS_ROLLOUT:-12}"
N_GPUS_TRAINING="${N_GPUS_TRAINING:-12}"
...
ROLLOUT_CHUNKED_PREFILL="${ROLLOUT_CHUNKED_PREFILL:-False}"
...
ROLLOUT_MAX_MODEL_LEN="${ROLLOUT_MAX_MODEL_LEN:-147456}"
...
ROLLOUT_RS_THRESHOLD="${ROLLOUT_RS_THRESHOLD:-0.997_1.003}"
...
python -m recipe.drkernel.main \
    ...
    actor_rollout_ref.ref.fsdp_config.offload_policy="${OFFLOAD_POLICY}" \
    actor_rollout_ref.ref.fsdp_config.reshard_after_forward=True \
    actor_rollout_ref.ref.fsdp_config.forward_prefetch="${FORWARD_PREFETCH}" \
    ...
    \
    `# algorithm — paper-faithful MRS via rollout_correction (NPU-recalibrated rs band).` \
    ...
    +ray_kwargs.ray_init.runtime_env.env_vars.USE_OPTIMIZED_MODEL="'0'" \
